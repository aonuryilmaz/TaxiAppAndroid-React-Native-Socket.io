...
//---------------------
const {
    GET_CURRENT_LOCATİON,
    GET_INPUT,
    TOGGLE_SEARCH_RESULT,
    GET_ADRESS_PREDICTIONS,
...
export function getSelectedAddress(payload){
    ...
    return(dispatch,store)=>{
        ...
        .then(()=>{
            ...
            if(store().home.selectedAddress.selectedPickUp &&store().home.selectedAddress.selectedPickUp ){
                request.get('https://maps.googleapis.com/maps/api/distancematrix/json').query({
                    ...
                    mode :'driving',
                    key:'AIzaSyDbnTTfIUnr0MkDZGm7qadQuR95O74vi3I'
                })
                .finish((error,res)=>{
                    dispatch({
            ...
            setTimeout(function(){
                if(store().home.selectedAddress.selectedPickUp &&store().home.selectedAddress.selectedPickUp ){
                    const fare=calculateFare(dummyNumbers.baseFare,dummyNumbers.timeRate,
...
export function bookCar(){
    return(dispatch,store)=>{
        ...
        request.post('http://10.0.2.2:3000/api/bookings')
        .send(payload)
        .finish((error,res)=>{
            dispatch({
                type:BOOK_CAR,
...
export function getNearByDrivers(){
    return (dispatch, store) => {
        request.get('http://10.0.2.2:3000/api/driverLocation').query({
            latitude:3.145909,
...
function handleGetAdressPredictions(state,action){
    return update(state,{
        predictions:{
            $set:action.payload
        }
...
function hanldeGetDistanceMatrix(state,action){
    return update(state,{
        distanceMatrix:{
            $set:action.payload
        }
...
const ACTION_HANDLERS = {
    GET_CURRENT_LOCATİON: handleGetCurrentLocation,
    GET_INPUT: handleGetInputData,
    TOGGLE_SEARCH_RESULT:handleToggleSearchResult,
    GET_ADRESS_PREDICTIONS:handleGetAdressPredictions,
    GET_SELECTED_ADDRESS:handleGetSelectedAddress,
    GET_DISTANCE_MATRIX:hanldeGetDistanceMatrix,
