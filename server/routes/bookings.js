...
var db=mongojs('mongodb://onur:1234@ds159254.mlab.com:59254/taxiapp',['bookings']);
...
router.put('/bookings/:id',function(req,res,next){
	...
	if(!booking.status){
		...
		db.bookings.update({_id:mongojs.ObjectId(req.params.id)},{$set:{
			...
			if(updatedBooking){
				db.bookings.findOne({_id:mongojs.ObjectId(req.params.id)},function(err,confirmedBooking){
					if(err){
						res.send(err)
					}
